...
const ADMIN_ONLY_EVENTS = new Set(['welcome', 'googleConnected', 'subscription', 'planReminder']);
...
const DEFAULT_TEMPLATES = {
  welcome: {
    ...
    body_html: `
      ...
      <p>Thanks for installing <strong>Google Console Analytics</strong> on <strong>{{shop_name}}</strong>. We're glad to have you.</p>
      <p style="color:${COLORS.subdued};">Open the app to connect your Google account and get started — it takes about a minute.</p>
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="{{app_url}}" style="display:inline-block;background:${COLORS.brand};color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;">Open the app →</a>
      </div>`,
    footer_html: `Need help? Reply to this email — a real human reads it.`,
  },

  planReminder: {
    subject: 'Pick a plan for {{shop_name}} and start your free trial',
    body_html: `
      <h1 style="margin:0 0 12px;font-size:22px;font-weight:700;">Ready to unlock the full app?</h1>
      <p>Hi! It looks like you installed <strong>Google Console Analytics</strong> on <strong>{{shop_name}}</strong> yesterday but haven't picked a plan yet.</p>
      <p>Every paid plan starts with a <strong>14-day free trial</strong> — no charge until the trial ends.</p>
      <h2 style="margin:24px 0 8px;font-size:16px;">What you get with a paid plan:</h2>
      <ul style="padding-left:20px;line-height:1.9;">
        <li>Full <strong>GA4</strong> + <strong>Search Console</strong> + <strong>Google Ads</strong> dashboards</li>
        <li><strong>Site Audit</strong> — automated crawls with score, errors, and warnings</li>
        <li><strong>AI Visibility</strong> — see how ChatGPT, Gemini and Perplexity describe your store</li>
        <li><strong>Content Creation</strong> + <strong>Product FAQs</strong> generated by AI</li>
        <li><strong>Stock alerts</strong>, <strong>weekly reports</strong>, <strong>CSV export</strong> and more</li>
      </ul>
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="{{app_url}}/billing" style="display:inline-block;background:${COLORS.brand};color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;">Choose a plan →</a>
      </div>
      <p style="color:${COLORS.subdued};font-size:13px;margin-top:16px;">Questions? Just reply to this email.</p>`,
    footer_html: `You're receiving this because you installed our app. We won't email you about plans again.`,
  },
...
// Welcome / plan-reminder don't reference the plan (the merchant hasn't
// picked one yet), so their token list is intentionally narrower.
const WELCOME_TOKENS = ['shop_name', 'app_url'];
const EVENT_META = {
  welcome:         { label: 'Welcome Email',                  description: 'Sent immediately when a shop installs the app.',           adminOnly: true,  availableTokens: WELCOME_TOKENS, sampleTokens: { shop_name: 'Acme Co', app_url: APP_URL } },
  planReminder:    { label: 'Plan Reminder Email',            description: 'Sent ~24h after install if no plan has been chosen yet.', adminOnly: true,  availableTokens: [...WELCOME_TOKENS, 'plan_features'], sampleTokens: { ...SAMPLE_PLAN, shop_name: 'Acme Co', app_url: APP_URL } },
